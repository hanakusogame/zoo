...
var Config = /** @class */ (function (_super) {
    ...
    function Config(scene, x, y) {
        ...
        var theme = 0;
        var btnTheme = new g.E({ scene: scene, x: 100, y: 200, width: 150, height: 45, touchable: true });
        base.append(btnTheme);
        var panels = [];
        for (var i = 0; i < 3; i++) {
            var panel = new Panel_1.Panel(scene, 50);
            panel.x = i * 40;
            panel.setNum(i);
            btnTheme.append(panel);
            panels.push(panel);
        }
        btnTheme.pointDown.add(function () {
            theme = (theme + 1) % 3;
            for (var i = 0; i < 3; i++) {
                panels[i].setTheme(theme);
            }
            if (_this.themeEvent !== undefined)
                _this.themeEvent(theme);
        });
        //ランキング表示
        var btnRank = new Button_1.Button(scene, ["ランキング"], 2, 258, 130, 45);
        base.append(btnRank);
        ...
        //閉じる
        var btnClose = new Button_1.Button(scene, ["閉じる"], 138, 258, 105, 45);
