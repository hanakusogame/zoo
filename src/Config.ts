...
export class Config extends g.FilledRect {
	...
	public themeEvent: (num: number) => void;
	...
	constructor(scene: g.Scene, x: number = 0, y: number = 0) {
		...
		let theme = 0;
		const btnTheme = new g.E({ scene: scene, x: 100, y: 200, width: 150, height: 45, touchable: true });
		base.append(btnTheme);
		const panels: Panel[] = [];
		for (let i = 0; i < 3; i++) {
			const panel = new Panel(scene, 50);
			panel.x = i * 40;
			panel.setNum(i);
			btnTheme.append(panel);
			panels.push(panel);
		}
		btnTheme.pointDown.add(() => {
			theme = (theme + 1) % 3;
			for (let i = 0; i < 3; i++) {
				panels[i].setTheme(theme);
			}
			if (this.themeEvent !== undefined) this.themeEvent(theme);
		});

		//ランキング表示
		const btnRank = new Button(scene, ["ランキング"], 2, 258, 130, 45);
		base.append(btnRank);
		btnRank.pushEvent = () => {
		...
		//閉じる
		const btnClose = new Button(scene, ["閉じる"], 138, 258, 105, 45);
